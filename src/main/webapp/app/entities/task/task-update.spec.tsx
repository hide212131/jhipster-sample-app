...
import translation from 'i18n/en/task.json';
...
import {
  ...
  preloadedState,
} from './task.test-samples';

const server = setupServer(
  rest.get('/api/tasks/1', (_req, res, ctx) => res(ctx.status(200), ctx.json({ id: 1, ...payloadData }))),
  rest.get('/api/tasks/2', (_req, res, ctx) => res(ctx.status(200), ctx.json({ id: 2, ...payloadDataPartical }))),
...
const renderComponent = initialEntry =>
  renderWithProviders(
    <>
      {/* Required for `navigate('/task');`. */}
      <Route index path="/task/*" element={<span>Navigated to Tasks</span>} />
...
describe('TaskUpdate Component Test Suite', () => {
  it('should render new TaskUpdate', async () => {
    // WHEN
  ...
  it('should POST with only required fields for new registration and handle success', async () => {
    ...
    server.use(
      rest.post('/api/tasks', async (req, res, ctx) => {
        try {
          ...
          return res(ctx.status(200), ctx.set('app-alert', 'myReactApp.task.created'), ctx.set('app-params', '1'), ctx.json(payloadData));
        } catch (e) {
          requestError = e;
    ...
    // THEN
    try {
      expect(await findByText(translate('myReactApp.task.created', { param: '1' }))).toBeInTheDocument();
    } catch (e) {
  ...
  it('should render update TaskUpdate', async () => {
  ...
  it('should PUT for update and succeed', async () => {
    ...
    let requestError = null;
    server.use(
      rest.put('/api/tasks/2', async (req, res, ctx) => {
        const payload = await req.json();
        try {
          expect(payload).toMatchObject({ id: 2, ...payloadData, ...payloadDataParticalUpdated });
          return res(ctx.status(200), ctx.set('app-alert', 'myReactApp.task.updated'), ctx.set('app-params', '2'), ctx.json(payloadData));
        } catch (e) {
          requestError = e;
          return res(ctx.status(500));
        }
    ...
    // THEN
    try {
      expect(await findByText(translate('myReactApp.task.updated', { param: '2' }))).toBeInTheDocument();
    } catch (e) {
      throw requestError ?? e;
    }
  });
  ...
  it(' should cancel and go back', async () => {
    ...
    // THEN
    expect(await findByText('Navigated to Tasks')).toBeInTheDocument();
  });
});
