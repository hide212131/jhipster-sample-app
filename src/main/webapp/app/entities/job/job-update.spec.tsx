...
import translation from 'i18n/en/job.json';
...
import {
  ...
  preloadedState,
} from './job.test-samples';

const server = setupServer(
  rest.get('/api/jobs/1', (_req, res, ctx) => res(ctx.status(200), ctx.json({ id: 1, ...payloadData }))),
  rest.get('/api/jobs/2', (_req, res, ctx) => res(ctx.status(200), ctx.json({ id: 2, ...payloadDataPartical }))),
  rest.get('/api/tasks', null),
...
const renderComponent = initialEntry =>
  renderWithProviders(
    <>
      {/* Required for `navigate('/job');`. */}
      <Route index path="/job/*" element={<span>Navigated to Jobs</span>} />
...
describe('JobUpdate Component Test Suite', () => {
  it('should render new JobUpdate', async () => {
    // WHEN
  ...
  it('should POST with only required fields for new registration and handle success', async () => {
    ...
    server.use(
      rest.post('/api/jobs', async (req, res, ctx) => {
        try {
          ...
          return res(ctx.status(200), ctx.set('app-alert', 'myReactApp.job.created'), ctx.set('app-params', '1'), ctx.json(payloadData));
        } catch (e) {
          requestError = e;
    ...
    // THEN
    try {
      expect(await findByText(translate('myReactApp.job.created', { param: '1' }))).toBeInTheDocument();
    } catch (e) {
  ...
  it('should render update JobUpdate', async () => {
  ...
  it('should PUT for update and succeed', async () => {
    ...
    let requestError = null;
    server.use(
      rest.put('/api/jobs/2', async (req, res, ctx) => {
        const payload = await req.json();
        try {
          expect(payload).toMatchObject({ id: 2, ...payloadData, ...payloadDataParticalUpdated });
          return res(ctx.status(200), ctx.set('app-alert', 'myReactApp.job.updated'), ctx.set('app-params', '2'), ctx.json(payloadData));
        } catch (e) {
          requestError = e;
          return res(ctx.status(500));
        }
    ...
    // THEN
    try {
      expect(await findByText(translate('myReactApp.job.updated', { param: '2' }))).toBeInTheDocument();
    } catch (e) {
      throw requestError ?? e;
    }
  });
  ...
  it(' should cancel and go back', async () => {
    ...
    // THEN
    expect(await findByText('Navigated to Jobs')).toBeInTheDocument();
  });
});
