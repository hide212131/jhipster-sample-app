...
import translation from 'i18n/en/location.json';
...
import {
  ...
  preloadedState,
} from './location.test-samples';

const server = setupServer(
  rest.get('/api/locations/1', (_req, res, ctx) => res(ctx.status(200), ctx.json({ id: 1, ...payloadData }))),
  rest.get('/api/locations/2', (_req, res, ctx) => res(ctx.status(200), ctx.json({ id: 2, ...payloadDataPartical }))),
...
const renderComponent = initialEntry =>
  renderWithProviders(
    <>
      {/* Required for `navigate('/location');`. */}
      <Route index path="/location/*" element={<span>Navigated to Locations</span>} />
...
describe('LocationUpdate Component Test Suite', () => {
  it('should render new LocationUpdate', async () => {
    // WHEN
  ...
  it('should POST with only required fields for new registration and handle success', async () => {
    ...
    server.use(
      rest.post('/api/locations', async (req, res, ctx) => {
        try {
          ...
          return res(
            ctx.status(200),
            ctx.set('app-alert', 'myReactApp.location.created'),
    ...
    // THEN
    try {
      expect(await findByText(translate('myReactApp.location.created', { param: '1' }))).toBeInTheDocument();
    } catch (e) {
  ...
  it('should render update LocationUpdate', async () => {
  ...
  it('should PUT for update and succeed', async () => {
    ...
    let requestError = null;
    server.use(
      rest.put('/api/locations/2', async (req, res, ctx) => {
        const payload = await req.json();
        try {
          expect(payload).toMatchObject({ id: 2, ...payloadData, ...payloadDataParticalUpdated });
          return res(
            ctx.status(200),
            ctx.set('app-alert', 'myReactApp.location.updated'),
    ...
    // THEN
    try {
      expect(await findByText(translate('myReactApp.location.updated', { param: '2' }))).toBeInTheDocument();
    } catch (e) {
      throw requestError ?? e;
    }
  });
  ...
  it(' should cancel and go back', async () => {
    ...
    // THEN
    expect(await findByText('Navigated to Locations')).toBeInTheDocument();
  });
});
