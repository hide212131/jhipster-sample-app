...
describe('Location e2e test', () => {
  ...
  describe('new Location page', () => {
    ...
    it('should create an instance of Location', () => {
      cy.get(`[data-cy="streetAddress"]`).type('carload sans');
      cy.get(`[data-cy="streetAddress"]`).should('have.value', 'carload sans');

      cy.get(`[data-cy="postalCode"]`).type('tomorrow anxious');
      cy.get(`[data-cy="postalCode"]`).should('have.value', 'tomorrow anxious');

      cy.get(`[data-cy="city"]`).type('南清水区');
      cy.get(`[data-cy="city"]`).should('have.value', '南清水区');

      cy.get(`[data-cy="stateProvince"]`).type('major');
      cy.get(`[data-cy="stateProvince"]`).should('have.value', 'major');
