...
describe('Blog e2e test', () => {
  ...
  const blogSample = { name: 'how', handle: 'drat annually tinted' };
  ...
  describe('new Blog page', () => {
    ...
    it('should create an instance of Blog', () => {
      cy.get(`[data-cy="name"]`).type('supervisor');
      cy.get(`[data-cy="name"]`).should('have.value', 'supervisor');

      cy.get(`[data-cy="handle"]`).type('gripping winding');
      cy.get(`[data-cy="handle"]`).should('have.value', 'gripping winding');
